...
import { Search, Building2, Loader2, CalendarDays } from "lucide-react";
...
const PeriodClientList = () => {
  ...
  const { data: clients, isLoading } = useQuery({
    queryKey: ['tenant-clients-periods', tenantId, searchTerm],
    queryFn: async () => {
      if (!tenantId) return [];
      let query = supabase
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 sm:py-20 px-4">
            ...
            <p className="text-sm text-muted-foreground">
              Cadastre clientes para gerenciar os períodos
            </p>
          </div>
        ) : (
