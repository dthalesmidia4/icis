...
import { Search, Building2, Loader2, Lightbulb } from "lucide-react";
import BackButton from "@/components/BackButton";
import { toast } from "sonner";

const StrategyClientList = () => {
  ...
  const { data: clients, isLoading } = useQuery({
    queryKey: ['tenant-clients-strategies', tenantId, searchTerm],
    queryFn: async () => {
      if (!tenantId) return [];
      let query = supabase
      ...
      <div className="container max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-12">
        ...
        <div className="mb-8 sm:mb-12 text-center relative">
          ...
          <p className="text-sm sm:text-lg text-muted-foreground">
            Selecione um cliente para acessar as estratégias
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 sm:py-20 px-4">
            <Lightbulb className="h-12 w-12 sm:h-16 sm:w-16 text-muted-foreground mx-auto mb-3 sm:mb-4" />
            <p className="text-base sm:text-lg font-medium mb-2">Nenhum cliente encontrado</p>
            <p className="text-sm text-muted-foreground">
              Cadastre clientes para acessar as estratégias
            </p>
          </div>
        ) : (
                ...
                <div className="relative p-6 sm:p-8 flex flex-col items-center justify-center text-center min-h-[160px] sm:min-h-[200px]">
                  {client.logo_url ? (
                    ...
                    <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-xl sm:rounded-2xl bg-primary flex items-center justify-center mb-3 sm:mb-4 group-hover:scale-110 transition-transform duration-300">
                      <Lightbulb className="w-6 h-6 sm:w-8 sm:h-8 text-primary-foreground" />
                    </div>
                  )}
...
export default StrategyClientList;
