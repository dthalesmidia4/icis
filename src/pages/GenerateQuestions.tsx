...
export default function GenerateQuestions() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('strategies').select('*').eq('company_id', selectedClient.id).eq('tenant_id', tenantId).order('created_at', {
        ascending: false
      }).limit(1).single();
  ...
  return <div className="min-h-screen bg-gradient-to-br from-background via-background to-accent/20">
      ...
      {selectedClient && !showQuestions && <div className="container max-w-4xl mx-auto py-8 px-4">
          ...
          <div className="bg-card rounded-lg shadow-lg p-8 space-y-8">
            ...
            <div className="space-y-4">
              ...
              {loadingStrategy ? <div className="text-center py-8 text-muted-foreground">
                  Carregando estratégia...
                </div> : !clientStrategy ? <Card className="p-8 text-center border-dashed">
                ...
                </Card> : <Card className="p-6 bg-gradient-to-br from-primary/5 to-secondary/5 border-2 border-primary/20">
            ...
            <div className="flex justify-end gap-3 pt-4">
              ...
              <Button onClick={handleGenerateQuestions} disabled={isGenerating || !clientStrategy} className="bg-gradient-to-r from-orange-500 to-orange-600 hover:opacity-90">
      ...
      {selectedClient && showQuestions && currentQuestion && <div className="container max-w-3xl mx-auto py-8 px-4">
          <Button variant="ghost" onClick={handleBack} className="mb-6">
            <ArrowLeft className="h-4 w-4 mr-2" />
            Voltar
          </Button>
          ...
          <div className="bg-card rounded-lg shadow-lg p-8 space-y-6">
              ...
              <div className="space-y-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <Textarea value={answers[currentQuestionIndex] || ''} onChange={e => handleAnswerChange(e.target.value)} placeholder="Digite sua resposta aqui..." className="min-h-[150px] resize-y" />
                  </div>
                </div>
              </div>
            </Card>
            ...
            <div className="flex justify-between items-center pt-4">
              ...
              <div className="flex gap-2">
                {questions.map((_, index) => <div key={index} className={`w-2 h-2 rounded-full transition-all ${answers[index]?.trim() ? 'bg-primary' : index === currentQuestionIndex ? 'bg-primary/50' : 'bg-muted'}`} />)}
              </div>

              {currentQuestionIndex === questions.length - 1 ? <Button onClick={handleSaveAnswers} className="bg-gradient-to-r from-emerald-500 to-emerald-600 hover:opacity-90">
                  <CheckCircle className="h-4 w-4 mr-2" />
                  Gerar Cronograma Final
                </Button> : <Button onClick={handleNextQuestion} disabled={currentQuestionIndex === questions.length - 1}>
                  Próxima
            ...
            {/* Save reminder */}
            {answers[currentQuestionIndex]?.trim() && <div className="text-center text-sm text-muted-foreground">
