...
export interface SystemsCommercialWorkspaceProps {
  /** Trava a tela numa única empresa/produto (uso embutido no Client Hub). */
  lockedCompanyId?: string | null;
  /** Remove o cabeçalho de página quando a tela vive dentro de outra. */
  embedded?: boolean;
  /** Agrupa as oportunidades pela carteira operacional (cidade). */
 ...
 *
 * A página `/comercial-sistemas` e a aba Comercial do Client Hub renderizam
 * ESTE mesmo componente: mesmos `systems_clients`, mesmo drawer, mesmo CRUD e
 * os mesmos `client_touchpoints`. Nada é duplicado; o Hub apenas trava a
 * empresa e agrupa por carteira operacional.
 */
export default function SystemsCommercialWorkspace({
  lockedCompanyId = null,
  embedded = false,
  groupByMarket = false,
}: SystemsCommercialWorkspaceProps = {}) {
  ...
  // Execução real por carteira (apenas na leitura agrupada).
  const [marketTouchpoints, setMarketTouchpoints] = useState<MarketTouchpoint[]>([]);
  // Agenda comercial da cidade, editada em modo `commercial`.
  ...
  // Empresa travada pelo Client Hub nunca é sobrescrita por filtro/URL.
  useEffect(() => {
    if (lockedCompanyId) setCompanyFilter(lockedCompanyId);
  }, [lockedCompanyId]);
  ...
  }, [searchParams, companies, markets, writeParams, lockedCompanyId]);
