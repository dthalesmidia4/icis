...
export interface UseRealtimeQuestionSessionsOptions {
...
/**
 * Assina `public.question_sessions` filtrado por tenant no server.
 * Aplica filtro local por company_id/client_id quando informado.
 */
export function useRealtimeQuestionSessions({
  tenantId,
  clientId,
  companyId,
  onChange,
  enabled = true,
  debounceMs = 250,
}: UseRealtimeQuestionSessionsOptions) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`rt-question-sessions-${tenantId}-${target}`)
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "question_sessions", filter: `tenant_id=eq.${tenantId}` },
        (payload: any) => {
