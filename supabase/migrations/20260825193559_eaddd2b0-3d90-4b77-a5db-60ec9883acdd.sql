-- =============================================================================
-- PAGAMENTO DA FATURA COM RECONCILIAÇÃO USD — ATÔMICO
--
-- Regra contábil: "câmbio do mês" é APENAS estimativa. O fato real de uma
-- compra em dólar é o par (USD original, BRL exatamente cobrado). O câmbio é
-- SEMPRE derivado desse par, individualmente, e recalculado no servidor —
-- nunca confiado ao cliente. Duas compras da mesma fatura podem ter câmbios
-- diferentes; não existe câmbio único de fatura.
--
-- Componentes NÃO recebem `paid_at`: a liquidação continua derivada da fatura.
-- As colunas `_enc` nunca são escritas aqui — os triggers de cifra cuidam disso.
-- =============================================================================
...
BEGIN
  ...
  -- Uma compra pertence a esta fatura quando cai entre o fechamento anterior
  -- (exclusivo) e o fechamento da competência (inclusivo).
  ...
  LOOP
    ...
    IF v_child_id IS NOT NULL THEN
      -- Só o fato financeiro; histórico e metadados preservados.
  ...
  -- ÚNICO update de liquidação: a própria fatura. Nenhum filho, nenhum vínculo,
  -- nenhum due_date.
  UPDATE public.finance_occurrences
