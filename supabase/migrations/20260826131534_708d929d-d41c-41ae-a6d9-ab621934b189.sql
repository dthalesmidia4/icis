...
CREATE OR REPLACE FUNCTION public.set_paid_finance_statement_iof(
  _occurrence_id uuid,
  _iof_brl numeric
)
RETURNS jsonb
...
BEGIN
  ...
  IF v_card.id IS NULL OR v_card.kind <> 'card' THEN
    RAISE EXCEPTION 'Somente faturas de cartão podem receber classificação de IOF';
  END IF;
  IF v_occ.paid_at IS NULL THEN
    RAISE EXCEPTION 'Informe o IOF retroativo apenas em faturas já pagas';
  END IF;
  ...
  UPDATE public.finance_occurrences
     SET iof_amount_brl = CASE WHEN v_iof > 0 THEN v_iof ELSE NULL END
   WHERE id = _occurrence_id;
...
REVOKE ALL ON FUNCTION public.set_paid_finance_statement_iof(uuid, numeric) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.set_paid_finance_statement_iof(uuid, numeric) FROM anon;
GRANT EXECUTE ON FUNCTION public.set_paid_finance_statement_iof(uuid, numeric) TO authenticated, service_role;